...
Resting orders are managed on every sweep. An exit sell still open after 45 seconds with the bid below its limit is canceled and re-placed at the current bid. An entry buy still open after two minutes is canceled. Pending orders are matched to the broker by broker order id; Robinhood's order list does not return `ref_id`.
